...
function hljsDefineGDScript(hljs) {
	var KEYWORDS = {
        /* General keywords */
		keyword:
			'in not or self void as assert breakpoint class class_name ' +
			'extends is func setget signal await yield const enum ' +
			'static var ',
        ...
        built_in:
            'Color void String Variant Object Array Dictionary ' +
            'bool int Resource void Color8 assert char convert ' +
            'dict_to_inst get_stack inst_to_dict is_instance_of ' +
            'len load preload print print_debug print_stack range ' +
            'type_exists PI TAU INF NAN ' +
            'AudioServer CameraServer ClassDB DisplayServer' +
            'Geometry2D Geometry3D GodotSharp IP' +
            'Input InputMap JavaClassWrapper JavaScriptBridge ' +
            'Marshalls NavigationMeshGenerator NavigationServer2D ' +
            'EditorInterface Engine EngineDebugger GDExtensionManager' +
            'InputEvent InputEventAction InputEventGesture ',
